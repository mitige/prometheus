...
#ifdef RG_ROCKETSIMCUDA_AVAILABLE
		GameState(const rsc::RocketSimCudaBatch& batch, int arenaIdx) : GameState() {
			UpdateFromCudaBatch(batch, arenaIdx, std::vector<Action>(batch.GetArenaInfo(arenaIdx).numCars), NULL);
		}
#endif
...
#ifdef RG_ROCKETSIMCUDA_AVAILABLE
		void UpdateFromCudaBatch(const rsc::RocketSimCudaBatch& batch, int arenaIdx, const std::vector<Action>& actions, GameState* prev);
		void UpdateFromCudaSnapshot(const rsc::ArenaInfo& arenaInfo, const rsc::BallState& ballState, const rsc::CarState* carStates, const rsc::BoostPadState* padStates, const std::vector<Action>& actions, GameState* prev);
#endif
