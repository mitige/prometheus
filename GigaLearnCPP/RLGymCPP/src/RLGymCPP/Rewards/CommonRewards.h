...
namespace RLGC {
	...
	// Penalizes the agent for rapidly changing an analog input from one step to the next.
	// Used to smooth continuous actions and prevent Bang-Bang control ("swerving").
	class ActionJitterPenalty : public Reward {
	public:
		virtual float GetReward(const Player& player, const GameState& state, bool isFinal) override {
			if (!player.prev)
				return 0;
			...
			float penalty = 0.0f;
			penalty += fabsf(player.prevAction.throttle - player.prev->prevAction.throttle);
			penalty += fabsf(player.prevAction.steer - player.prev->prevAction.steer);
			penalty += fabsf(player.prevAction.pitch - player.prev->prevAction.pitch);
			penalty += fabsf(player.prevAction.yaw - player.prev->prevAction.yaw);
			penalty += fabsf(player.prevAction.roll - player.prev->prevAction.roll);

			// Max swing per axis is 2.0 (-1 to 1). 5 axes = 10.0 max total swing.
			return -penalty / 10.0f; 
		}
	};
}