#include "UnlimitedBoostState.h"
#include "../Math.h"
using RocketSim::Math::RandFloat;
using RLGC::Math::RandVec;

static Vec RandNormVec() {
    return RandVec(Vec(-1, -1, -1), Vec(1, 1, 1)).Normalized();
}

void RLGC::UnlimitedBoostState::ResetArena(Arena* arena) {
    MutatorConfig mutators = arena->GetMutatorConfig();
    mutators.boostUsedPerSecond = 0;
    mutators.carSpawnBoostAmount = 100;
    arena->SetMutatorConfig(mutators);

    arena->ResetToRandomKickoff();

    constexpr float
        X_MAX = 3500,
        Y_MAX = 4000,
        Z_MAX = 2000,
        BALL_Z_MIN = 1500.0f, // minimal spawn height
        BALL_Z_MAX = 2000.0f, // maximal spawn height
        CAR_Z_MIN = 150,
        PITCH_MAX = M_PI / 2,
        YAW_MAX = M_PI,
        ROLL_MAX = M_PI,
        ANGVEL_MAX = 5.5f;

    {
        BallState bs = {};
        bs.pos = Math::RandVec(Vec(-X_MAX, -Y_MAX, BALL_Z_MIN), Vec(X_MAX, Y_MAX, BALL_Z_MAX));
        if (randBallSpeed) {
            Vec vel = RandNormVec() * RandFloat(0, 4000);
            vel.z = RandFloat(-50.0f, 50.0f);
            bs.vel = vel;
            bs.angVel = Math::RandVec(Vec(-4, -4, -4), Vec(4, 4, 4));
        }
        arena->ball->SetState(bs);
    }

    for (Car* car : arena->_cars) {
        CarState cs = {};
        cs.pos = Math::RandVec(Vec(-X_MAX, -Y_MAX, CAR_Z_MIN), Vec(X_MAX, Y_MAX, Z_MAX));
        if (randCarSpeed) {
            Vec randVelDir = Math::RandVec(Vec(-1, -1, -1), Vec(1, 1, 1)).Normalized();
            cs.vel = RandNormVec() * RandFloat(0, RLConst::CAR_MAX_SPEED);
            cs.angVel = RandNormVec() * ANGVEL_MAX;
        }
        Angle angle = Angle(RandFloat(-YAW_MAX, YAW_MAX), RandFloat(-PITCH_MAX, PITCH_MAX), RandFloat(-ROLL_MAX, ROLL_MAX));
        bool onGround = carsOnGround ? true : (RandFloat() > 0.5f);
        if (onGround) {
            cs.pos.z = 17;
            angle.pitch = angle.roll = 0;
            cs.vel.z = 0;
            cs.angVel = {};
        }
        cs.rotMat = angle.ToRotMat();
        cs.boost = 100.0f;
        car->SetState(cs);
    }
}