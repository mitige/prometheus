#include "StallAction.h"

RLGC::StallAction::StallAction() {
	constexpr float
		// Boolean input
		R_B[] = { 0, 1 },
	...
	for (float pitch : R_F) {
		for (float yaw : R_F) {
			for (float roll : R_F) {
				for (float jump : R_B) {
					for (float boost : R_B) {
						bool is_stall = (jump == 1 && pitch == 0 && ((roll == 1 && yaw == -1) || (roll == -1 && yaw == 1)));

						// Only need roll for sideflip, except for stalls
						if (jump == 1 && yaw != 0 && !is_stall)
							continue;
...
std::vector<uint8_t> RLGC::StallAction::GetActionMask(const Player& player, const GameState& state) {
