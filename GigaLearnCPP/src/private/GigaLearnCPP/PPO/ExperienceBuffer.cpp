...
GGL::ExperienceTensors GGL::ExperienceBuffer::_GetSamples(const int64_t* indices, size_t size) const {
	...
	Tensor tIndices = torch::tensor(IList(indices, indices + size), torch::TensorOptions().dtype(torch::kInt64));
	if (data.states.is_cuda())
		tIndices = tIndices.to(data.states.device(), true, true);
