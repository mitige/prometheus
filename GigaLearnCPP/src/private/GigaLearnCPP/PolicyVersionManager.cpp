...
void GGL::PolicyVersionManager::RunSkillMatches(PPOLearner* ppo, Report& report) {
	...
	for (float t = 0; 
		...
		if (isContinuous) {
			...
			auto combinedActions = std::vector<float>(skill.envSet->state.numPlayers * actionDim, 0.0f);
			...
			skill.envSet->Sync();
			skill.envSet->StepSecondHalfContinuous(combinedActions, actionDim, false);
		} else {
